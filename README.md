...
alert icmp: Perintahkan Suricata memantau protokol ICMP (protokol untuk Ping).

any any -> any any:



any (kiri): Dari IP manapun, Port berapapun.

->: Menuju ke.

any (kanan): Ke IP manapun, Port berapapun.

Merupakan rules basic untuk deteksi ping atau ICMP jadi nantinya jika ada dari sumber manapun akan muncul pada log suricata nyua



Tujuan: Memastikan engine Suricata mengetahui dan bisa melihat trafik jaringan.
...
```
alert tcp any any -> $RISET_SERVER any (msg:"[WARN] Port Scanning ..."; flags:S; flow:stateless; sid:100002; rev:1;)
```

alert tcp: Memantau protokol TCP.

flags:S (main key): Untukn mendeteksi paket yang memiliki flag TCP SYN (Synchronize).


Dalam TCP Handshake, SYN adalah  seperti "ketukan pintu" (proccess synchronize) pertama. Nmap bekerja dengan mengirim ribuan reqeust (SYN) ke ribuan port berbeda.

flow:stateless: Suricata tidak perlu menunggu koneksi jadi. Cukup lihat satu paket SYN saja, langsung teriak.

Logika: "Jika ada orang mengetuk pintu (SYN) ke arah Server Riset, beritahu saya." Karena kita menghapus threshold, satu ketukan pun dianggap scanning.

***



Untukn rules bruteforce SSH

```
alert tcp any any -> $RISET_SERVER 22 (... threshold: type both, track by_src, count 2, seconds 10; ...)
```

-> \$RISET\_SERVER 22: Targetnya spesifik ke Port 22 (Port SSH).

threshold: ... count 2, seconds 10 (Logic yang cukup matematis):

&#x20;Suricata akan menghitung:

"Apakah IP Pengirim ini (track by\_src) mengirim paket SYN ke port 22 sebanyak 2 kali atau lebih dalam waktu 10 detik?"

Logika: Login normal biasanya hanya membutuhkan 1 koneksi atau 1 kali percobaan dan mungkin di beberapa kasus maks hanya 3 kali(biasnya ada yang typo creds). Jika ada 2 koneksi cepat dalam 10 detik, asumsinya ini adalah bot/script (Hydra) yang sedang mencoba menebak password.
...
```
alert tcp any any -> any any (msg:"[CRITICAL] ..."; content:"confidential"; nocase; sid:9999999; rev:1;)
```


tcp any any -> any any: Suricata memantau keseluruhan lalu lintas TCP dari siapa saja ke siapa saja (Mengabaikan filter IP \$RISET\_SERVER yang kadang salah setting).

content:"confidential": Ini adalah fitur Deep Packet Inspection (DPI).




Suricata membongkar isi paket (payload).

Dia mencari susunan huruf c-o-n-f-i-d-e-n-t-i-a-l.

nocase: No Case Sensitive. Artinya dia akan mendeteksi "Confidential", "CONFIDENTIAL", atau "CoNfiDenTiaL".

Logika: "Saya tidak peduli siapa pengirimnya, saya tidak peduli tujuannya ke mana. Kalau ada paket lewat membawa dokumen bertuliskan 'confidential', itu brarti pasti pencurian data, kurangnya ialah mungkin paket dengan nama selain confidential akan masih terjadi salah error di sini
...
Pada firewall perlu di edit sebagai berikut karena by default firewall kami menggunakan ZBF method yang strict dalam defense beberapa port tertentu, sehingga gunakan konfigurasi berikut pada firewall yang ada : 

```
# 1. Pastikan IP Forwarding aktif (Router Mode)
sysctl -w net.ipv4.ip_forward=1

# 2. Hapus semua aturan lama yang mungkin memblokir TCP
iptables -F
iptables -X
iptables -t nat -F

# 3. KUNCI SUKSES: Izinkan paket menyeberang (Forwarding)
iptables -P FORWARD ACCEPT
iptables -P INPUT ACCEPT
iptables -P OUTPUT ACCEPT

# 4. Pasang NAT "Sapu Jagat" (Agar rute balik aman)
iptables -t nat -A POSTROUTING -j MASQUERADE
```



Nyalakan service SSH pada riset
...
# Kirim Ping tapi isinya data rahasia
ping -p 636f6e666964656e7469616c 192.168.4.1
# (Hex code itu artinya "confidential")

(Instalasi netcat pada akademik1)
curl -v -X POST -d "rahasia=confidential" http://192.168.7.2/

hydra -l root -p badpassword -t 4 192.168.8.2 ssh
hydra -l root -P pass.txt 192.168.8.2 ssh -V (Kalau pakai wordlist)
```
